...
- **`--output-mode per-building` ≠ building-major layout.** It writes integer
  ...
  batch per building (split the config with `tools/campus/split_campus_config.py`; see
  `configs/_campus_base_split/`, driven by the generic `tools/campus/run_campus.sh`).
...
- **Post-run analysis:** `tools/campus/analyze_overnight.py` (single building,
  slight-vs-moderate trees) and `tools/campus/analyze_campus.py` (building-major
...
- **`tools/README.md`** — index of the operational scripts, grouped `paper/`,
  `validation/`, `campus/`, `data_prep/`. None are part of the installed package;
  the flat module layout inside each subfolder is load-bearing for the
  `spec_from_file_location` imports in `tests/` and the `sys.path` inserts in
  `tools/paper/repro_paper.py`.
