...
Usage:
    uv run python tools/validation/validate_calibration.py \\
...
SOURCE_SPECS = {
    ...
    # tools/data_prep/ingest_evwatts.py (session⋈evse, venue="Business Office").
    "evwatts": {
        "policy": "evwatts",
...
# (CV(RMSE)/NMBE vs ComStock) lives in tools/validation/validate_buildingload.py; S5 keeps
...
def _load_reference_bands(path: Path = REFERENCE_BANDS_PATH) -> dict | None:
    ...
    if not path.exists():
        log.warning(
            ...
            "tools/data_prep/fetch_buildingload_reference.py; falling back to coarse "
            "smoke bands.", path,
        )
        return None
    import json
...
def s5_buildingload(output_root: Path) -> pd.DataFrame:
    ...
    tools/validation/validate_buildingload.py.
    """
...
def write_results_md(output_root: Path) -> None:
    ...
    w("")
    w("> **Auto-generated by `tools/validation/validate_calibration.py`. Do not edit by hand.**  ")
    w("> Regenerate (full paper source list): `uv run python tools/paper/repro_paper.py "
      "--steps calibration`  ")
    ...
    if not s5.empty:
        ...
        w(f"- **S5 building load (real-data)** — {ww}/{len(s5)} within the NREL "
          ...
          "metrics in `tools/validation/validate_buildingload.py`).")
    if not s6.empty:
    ...
    if not s5.empty:
        ...
        w("_Coarse real-data smoke test. The weekday/weekend band is derived "
          ...
          "`peak_kw_scaling` off) lives in `tools/validation/validate_buildingload.py` — "
    ...
    # S5b — rigorous G14 metrics from tools/validation/validate_buildingload.py, if its
