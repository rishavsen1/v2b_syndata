...
<main>
  ...
  <section class="muted">
    ...
    <p><b>Notes:</b> EV charging requirements are guaranteed reachable within each session's dwell at the charger rate (D5, strict — no session's required SoC exceeds what it can physically charge). <code>weather_data.csv</code> is at native EPW <b>hourly</b> resolution while the other timeseries are 15-min — resample weather to 15-min before joining on a common index.</p>
  </section>
</main>"""
