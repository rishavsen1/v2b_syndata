...
<main>
  ...
  <section class="muted">
    <h2>Provenance</h2>
    <p>Generated by <code>generate-multi</code> from per-building splits of <code>configs/campus_base.yaml</code> (<code>configs/_campus_base_split/b*.yaml</code>), one batch run per building into <code>data/output/campus_base/b1 … b{nb}</code>; <code>--start-month 2024-01 --end-month 2024-06 --samples-per-month 100 --noise-profile clean</code>; per-building <code>weather_profile</code> (strong). Per-sample summaries in <code>analysis_summary.csv</code>. Every number is reproducible from <code>seed + config</code>.</p>
    <p><b>Notes:</b> EV charging requirements are guaranteed reachable within each session's dwell at the charger rate (D5, strict). <code>weather_data.csv</code> is native EPW <b>hourly</b> while other timeseries are 15-min — resample before joining. Peak scaling is <b>on</b> so each building's peak is renormalized to its <code>building_load.peak_kw</code>.</p>
  </section>
</main>"""
