...
units). Soft advisories are disclosed rather than suppressed: units may carry
warnings for charger-concurrency near saturation (a scenario-design property)
...
\paragraph{Compute.}
One dataset unit (one building-month) costs ${\sim}117$\,s on one core,
dominated by EnergyPlus; the 18{,}000-unit corpus took ${\approx}580$
CPU-hours (${\approx}8$\,h wall-clock on 30 workers). Single-scenario
generation for interactive use completes in roughly two minutes.
