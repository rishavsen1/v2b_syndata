...
network-scale cohort (1{,}231 drivers in the fitted workplace cohort;
481\,kW observed peak versus 26\,kW synthetic). Shape transfer again reaches parity---the
...
extrapolate to a fleet sixty times larger, and a 20-vehicle aggregate is
intrinsically spikier than a 1{,}231-driver one. We report both
rows (Table~\ref{tab:tstr}, Fig.~\ref{fig:tstr}) and read them as a
\emph{scale-shift} result: the
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig5_tstr.pdf}
  \caption{TSTR/TRTR error ratios of Table~\ref{tab:tstr} (log scale; dashed
  line is parity). ACN same-cohort transfer sits at or below parity;
  ElaadNL cross-scale transfer reaches parity only in the calendar-only,
  unit-mean-normalized regime, isolating shape transfer from magnitude
  transfer.}
  \label{fig:tstr}
\end{figure}
