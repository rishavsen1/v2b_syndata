...
committed scripts (`tools/paper/repro_paper.py` — the one-shot reproduction driver —
plus `tools/validation/validate_calibration.py`, `tools/validation/validate_buildingload.py`,
`tools/validation/validate_pv.py`, `tools/paper/tstr_forecasting.py`, `tools/validation/model_eval.py`);
...
> `tools/data_prep/ingest_evwatts.py`, calibrated 2026-06-28) — real-data fidelity
...
#1; `tools/validation/validate_buildingload.py`). PV is a deterministic PVWatts-style curve
...
- **TSTR utility (train-on-synthetic, test-on-real).** A short-horizon load /
  ...
  **real** data (`data/tstr/*.json`, regenerated by `tools/paper/repro_paper.py`):
...
(`tools/paper/repro_paper.py` → `docs/experiments/PAPER_NUMBERS.md`). Determinism
