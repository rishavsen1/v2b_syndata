...
| 8 | P1 | 3 | `tools/validate_pv.py`: compare `pv_model.pv_ac_series` to **PVWatts v8 API / SAM** (<5% annual error; G14 hourly). | Cheap, high-credibility validation of a deterministic model. | ✅ 2026-07-08 (WS-B): PySAM on identical EPW; +1.27% annual, CV(RMSE) 15.1%, r 0.994. |
...
| 13 | P2 | all | Re-run `calibrate` → `validate_calibration` → `model_eval` → benchmark; commit refreshed results; reconcile `depart_soc_mu` 85-vs-50 doc contradiction (tracker O4). | Every paper number regenerable from committed scripts. | ✅ 2026-07-09/11 (WS-F+): tools/repro_paper.py (calibration+buildingload+2×TSTR+ablation+dispatch+scale → PAPER_NUMBERS.md, bit-identical double runs); O4 closed. |
