...
- S5 check (`tools/validation/validate_calibration.py:612-661`) compares against bands
...
| 7 | ~~P1~~ | 3 | ~~Add battery dispatch → `battery_dispatch.csv`~~ | ~~"missing operational behavior"~~ | ↩️ **REVERTED (out of scope, 2026-06-29).** Battery dispatch is a downstream control *decision*, not a generator output — exactly parallel to `cars.csv` shipping specs, not charging schedules. The "missing operational behavior" framing was a scope error: a generator should emit the problem instance (load, prices, DER **specs**), not solve it. `battery.csv` keeps specs. If a reference dispatch is ever wanted it belongs in `bench/` as a labeled **baseline**, never as a dataset CSV. |
| 8 | P1 | 3 | `tools/validation/validate_pv.py`: compare `pv_model.pv_ac_series` to **PVWatts v8 API / SAM** (<5% annual error; G14 hourly). | Cheap, high-credibility validation of a deterministic model. | ✅ 2026-07-08 (WS-B): PySAM on identical EPW; +1.27% annual, CV(RMSE) 15.1%, r 0.994. |
| 9 | P2 | 3 | Map each tariff to an OpenEI **URDB** rate ID; validate peak/off-peak `$/kWh` + `peak_window`. | Makes prices traceably real. | ☐ |
| 10 | P2 | 3 | Bound/fit DR magnitudes to published **CAISO / PG&E CBP/BIP/ELRP** commitments; else caveat explicitly. | Replaces a no-data prior with grounded ranges. | ✅ 2026-07-08 (WS-D): BIP tariff-derived, CBP evaluation-consistent, ELRP stylized; docs/experiments/dr_magnitude_grounding.md; config re-bounds deferred (W11/W12). |
| 11 | P2 | 1 | Wire held-out KS into calibration (tracker F2/F3); report bootstrap CIs on KS/W₁. | Every fidelity number is in-sample today. | ✅ (WS-A 2026-07-08). Seeded bootstrap (B=1000, seed 20260708, per-cell hashed sub-streams; bit-reproducible) → 95% CIs on S1 KS/W₁ in `CALIBRATION_RESULTS.md` + `docs/experiments/s1_fidelity_cis.csv`. F3 protocol repaired: S3 holdout refits the *shipped* family (mixture where calibration ships one); per-cell Δ + refit-vs-shipped family reported. Honest finding: family-matched holdout worsens some cells (acn/regular_charger arrival +0.234; office001 +0.425 at n_test=62) — cohort-shift/small-n caveats in the doc. |
...
| 13 | **P0** | 2 | **Regenerate ALL frozen-evidence artifacts through `tools/paper/repro_paper.py` after the 2026-08-18 model revision.** The generative model changed materially: same-local-day session filter, κ removed (φ-only 3-bin regions, erratic/rare_inconsistent folded), per-bin φ Beta, dwell.k ceiling 5→15 (mixtures now ship in every ACN cell), per-region delivered-energy LogNormal with `required_soc_at_depart` DERIVED (soc_arrival no longer fitted; D7 bypassed on the calibrated path), truncated Beta(2,3) arrival prior, proportional SoC chain. **Stale until regenerated:** `docs/experiments/family_selection.{md,csv}`, `mixture_ablation.{md,csv}`, `PAPER_NUMBERS.md`, `source_vs_generated.png`, `s1_fidelity_cis.csv`, `docs/CALIBRATION_RESULTS.md`, and every KS number cited in the paper from tasks 4–6/11 above. Numbers in the paper must not cite the old artifacts. Evidence-freeze policy applies: regenerate via the driver, never by hand. | The paper's claims→evidence map breaks silently otherwise: committed populations.yaml no longer reproduces from the old pipeline. | ☐ **BLOCKING for any paper number** |
| 13 | P2 | all | Re-run `calibrate` → `validate_calibration` → `model_eval` → benchmark; commit refreshed results; reconcile `depart_soc_mu` 85-vs-50 doc contradiction (tracker O4). | Every paper number regenerable from committed scripts. | ✅ 2026-07-09/11 (WS-F+): tools/paper/repro_paper.py (calibration+buildingload+2×TSTR+ablation+dispatch+scale → PAPER_NUMBERS.md, bit-identical double runs); O4 closed. |
...
- ~~`verify_sweep.py` references metric columns absent from `MetricsResult`~~ ✅ fixed + proven end-to-end (WS-E, 2026-07-08).
- ~~Benchmark is V1G-only~~ ✅ V2B dispatch LP baseline added (2026-07-11): 29.4% peak shave with discharge+storage on corpus unit b1/JUL2024/0; ACN-Sim demand cross-check. ACN-Sim scheduler itself remains V1G (disclosed §8).
