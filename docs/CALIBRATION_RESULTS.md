...
> **Auto-generated by `tools/validation/validate_calibration.py`. Do not edit by hand.**  
> Regenerate (full paper source list): `uv run python tools/paper/repro_paper.py --steps calibration`  
...
- **S5 building load (real-data)** — 2/4 within the NREL ComStock weekday/weekend band (coarse smoke test; rigorous G14 metrics in `tools/validation/validate_buildingload.py`).
...
_Generator's raw single-prototype EnergyPlus load vs the ComStock stock-average for each (archetype,size), from `tools/validation/validate_buildingload.py` (`data/buildingload_reference/validation_metrics.json`). G14 thresholds: CV(RMSE) ≤ 30 %, |NMBE| ≤ 10 %._
